...
class Constrained_Random_Table_Generation():
    def __init__(self, schema: dict, sizes, fake=None, raw_cstr: dict={}, seed=2333, is_extreme: bool=False) -> None:
        ...
        # init parser
        sizes = [sizes[0] for _ in range(len(schema['Tables']))]
    ...
    def get_bound(self, _type, _range:list=[], is_extreme: bool=False):
        ...
        if not is_extreme:
            if _type == 'int':
                ...
                lower = datetime.date(1900,1,1)
                upper = datetime.date(2100,12,30)
            elif _type == 'time':
                lower = datetime.datetime(1900,1,1,0,0,0)
                upper = datetime.datetime(2100,12,31,23,59,59)
            elif _type == 'numeric' or _type == 'decimal':
                lower = 0.0
                upper = 2147483647.0
            if len(_range) != 0:
            ...
            if unique:
                ...
                def generator(size):
                    if size >= 30:
                        ...
                        output = [self.fake.unique.name() for _ in range(size)]
                    return output
                return generator
        elif _type == 'numeric' or _type == 'decimal':
            if unique:
                return lambda k: [float(num) for num in random.sample(range(int(lower), int(upper)+1), k)]
            else:
            ...
            if cstr['operator'] == 'range':
                if cstr['info']['discrete']:
                    ...
                    if _type == 'int':
                        ...
                        upper = cstr['info']['upper'].split(' ')[0] if cstr['info']['upper'] is not None else '2100-12-31'
                        lower = cstr['info']['lower'].split(' ')[0] if cstr['info']['lower'] is not None else '1900-1-1'
